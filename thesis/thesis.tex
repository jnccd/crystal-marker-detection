...
\begin{figure}
  \caption{Hyper Parameter Optimization History for \ac{YOLO}v5s and \texttt{gpv2}, where after each improvement to the highest score, the largest change in percent of the defined search space range to the parameters from the last highest score to the new highest score is visualized as annotations}
...
To find an optimal combination of parameters, I did hyperparameter optimizations as described in \autoref{sec:hyper_opt}. The optimal found augmentation parameters using \ac{YOLO}v5s, \texttt{gpv2} and \ac{SAHI} can be seen in \autoref{fig:optimal_augment_params} and the change in score over the training runs can be seen in \autoref{fig:hyp_hist}. 
...
\begin{figure}
  \caption{Hyper Parameter Optimization History for \ac{YOLO}v5x and \texttt{gpv3}, where after each improvement to the highest score, the largest change in percent of the defined search space range to the parameters from the last highest score to the new highest score is visualized as annotations}
