...
The rotation augmentation is the one which improved the scores the most, as seen in \autoref{fig:zero_rot_plot}. Here more usage rate is needed before the score gains level out. The correlation for the VOC 2010 score is 54\% and for the COCO score is 21\% here. This may indicate that the precise placement of the bounding boxes suffers in the cases, where the marker could be easily detected before when using more rotation augmentation. However at this stage in development my biggest focus is to detect more markers, so the VOC 2010 score has more weight.
...
Random Crop v2 had the worst impact on scores, as seen in \autoref{fig:zero_rc2_plot}. This may be because it zooms too far into the image. Default Random Crop did not show similar impact on the scores.
...
Other augmentations, such as Label Move, showed a slight score increase at low usage rates and 
...
\begin{figure}
  \caption{Hyper Parameter Optimization History, where after each improvement to the highest score the largest change to the parameters from the last highest score is visualized as annotations}
...
With more or more stable tapes, more training data which is closer to the real world usecase could be created. Since the data transformations, the training and evaluation are automated in the scripts developed during this work, new models could quickly be developed using the new data.
