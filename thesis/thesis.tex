...
\ac{YOLO}v2 improves on \ac{YOLO} with the introduction of anchor boxes, allowing the network to detect objects in regions of the image instead of the whole image at once \cite{redmon2017yolo9000}. Furthermore all fully connected layers are removed from the \ac{YOLO}v2 network architecture. \ac{YOLO}v2 also introduced multi-scale training, meaning it can adapt its network size to different input image sizes from $320 \times 320$ to $608 \times 608$.

\ac{YOLO}v3 mainly improves the network architecture by combining approaches from \ac{YOLO}v2 and Darknet-19 to create Darknet-53, which got its name from the 53 convolutional layers it contains \cite{redmon2018yolov3}. Darknet-53 proved to be a more efficient backbone network. \ac{YOLO}v3 also introduced spatial pyramid pooling, which allowed it to detect bounding boxes at different scales more effectively.

\section{Yolov5}

\ac{YOLO}v5 was developed by Ultralytics instead of the developers of the first three \ac{YOLO} versions and further increased the mAP detection score on the COCO dataset. \ac{YOLO}v5 also made hyperparameter optimization easier and added quality of life features.
