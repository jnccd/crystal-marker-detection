...
In this this work, different algorithmic approaches for the detection of the \ac{ArUco} fiducial marker structures are analyzed, such that the sensitivity to artifacts in the marker can be reduced. Since the tapes should be used as \acp{POCT} by patients, the technology developed here is supposed to run on patients smart phones. While smart phones are easily carryable, they come with the drawback of not having much space or energy for large \acp{GPU}, which limits the choice of neural network models.
