...
When filtering for only polygons with four vertices, the small polygon is detected on one scale, as seen on \figureref{fig:classic_base_img_opencv} \textbf{(a)}. The better detection accuracy here may be due to cherry picked parameters. Without the filter in \figureref{fig:classic_base_img_opencv} \textbf{(b)}, the problems of the polygon detection become apparent, as the contours of the markers in the image are not complete, which may in part explain the findings in \autoref{sec:aruco_det}.
...
% TODO: Improve this part and get some good results to show here, on GPV2 maybe and without SAHI or other inference filters
...
The best fitted \ac{YOLO}v5s model created during the hyper parameter optimization on \texttt{gpv2} scores 49.13\% on the VOC 2010 \ac{mAP} and 26.4\% on the COCO \ac{mAP}. The best fitted \ac{YOLO}v5x model from the hyperparameter optimization on \texttt{gpv3} scores 53.04\% on the VOC 2010 \ac{mAP} and 33.4\% on the COCO \ac{mAP}. The score difference of the two models can be mainly explained by fewer wrong detections, as seen in \figureref{fig:confusion_matrix_compare}. This may indicate that certain markers in the final test data do not appear in the training data and therefore will not be detected with any model fitted on that training data.

\begin{figure}
  \centering
  \subcaptionbox{\ac{YOLO}v5s confusion matrix on final test set}
     {\includegraphics[width=0.49\textwidth]{image/yolov5s_hyp_search_confusion_matrix}}
  \subcaptionbox{\ac{YOLO}v5x confusion matrix on final test set}
     {\includegraphics[width=0.49\textwidth]{image/yolov5x_hyp_search_confusion_matrix}}
  \caption{Confusion matrices of the best \ac{YOLO}v5s and \ac{YOLO}v5x model fitted during hyperparameter search.}
...
\begin{table}
  \begin{tabular}{ c c c }
   ...
   \hline
  \end{tabular}
  \caption{\label{tab:pet_valdata_results}Comparison of Sinkhorn loss and mean point distance on validation data for different point detection approaches. The image size is 160 x 160 pixels.}
\end{table}
...
Based on the comparison from Terven et al. and Wang et al., \ac{YOLO}v7 models may be promising since they achieve a moderately better \ac{COCO} \ac{mAP} score than \ac{YOLO}v5 \cite{terven2023comprehensive, wang2023yolov7}.
...
\begin{figure}
  ...
  \includegraphics[width=\textwidth]{image/instructions-paper}
  \label{fig:instructions_paper}
\end{figure}

\begin{figure}
  \caption{The instructions paper page 2 for the human baseline study.}
  \includegraphics[page=2,width=\textwidth]{image/instructions-paper}
