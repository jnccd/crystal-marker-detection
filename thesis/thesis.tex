...
\begin{figure}
  \caption{Pixel comparisons between the \ac{ArUco} marker image and the theresholded image snippet}
...
To get a score that I can compare the evaluation results to, I gathered scores that humans would achieve on this detection task. To do this, I distributed the images of the final test set and a instructions paper on the Kiel University WeTalk isntance and Discord. The instructions paper contains a general introduction to the task, a perfect example marker, example training images with labels and step by step instructions for the labeling process. The labeling was done on makesense.ai\footnote{\url{https://www.makesense.ai}, accessed on 11.11.2023}, since it does not require an installation and instead works in the browser, while also containing all nessecary tools and, since the dataset is small enough to be distributed directly as files. In total there were 7 participats who returned their marked labels. For the evaluation I treated the resulting label data the same as the network output and generated \ac{mAP} scores using the same scripts. The resulting \ac{mAP} scores can be seen in \autoref{tab:human_test_result}.
...
In this section I describe some of the fixed parameters during the testing. Since this work introduces many varaibles, such as model type, augmentation parameters and model specific hyperparameters, some of them need to be fixed to keep the scope simple enough. These constraints are based on eariler and smaller scale tests.

% TODO: explain why: gpv2, yolov5s (informal?), plots look very bad but only proof Ive got so lets go
...
During inital testing of the datasets using yolov5s as the model to train, there was an increase in \ac{mAP} score from the bigger \texttt{gpv1} dataset to the smaller \texttt{gpv2} dataset, where more samples where sorted out based on image quality. However, while the scores without augmentation further increased with the dataset version, for \texttt{gpv3} the maximum scores did not increase. While the stagnation of scores can also stem from a combination of augmentations that were not suitable for \texttt{gpv3}, I mainly kept testing with \texttt{gpv2} instead, such that newer tests would be better comparable with older tests. The results can be seen in \autoref{fig:initial_dataset_compare}. % TODO: redo this with newer runs?
...
Of the available inference parameters I mainly used \ac{SAHI} in the tests, since the markers in the testset were small and \ac{SAHI} incrased the \ac{mAP} score in initial tests, or none.
...
To find an optimal combination of parameters, I did hyperparameter optimizations as described in \autoref{sec:hyper_opt}. The optimal found augmentation paramters using \ac{YOLO}v5, \texttt{gpv2} and \ac{SAHI} can be seen in \autoref{fig:optimal_augment_params}. The learning cutoff point found by the search is at 347 epochs, which indicates that longer learning can benefit the score in this case even though the training and testing images are not of the same domain and not completely similar. The run achieved a Pascal VOC 2010 \ac{mAP} score of 70.78\%.

% TODO: add params graph? or table

\section{Augmenting from Optimum}

While the results of the hyperparameter optimization run show what works, they do not nessecarily indicate which augmentations have the biggest impact on the score. For this purpose I chose to do follow up ensemble runs, where one parameter again changes from 0 to its maximum value and all other parameters are fixed at their optimal value.

% TODO: add text, show graphs

\section{Inference Parameters}

Based on the training run from the hyperparameter optimzation, I tested different combinations of inference parameters as seen in \autoref{tab:inference_parameters}. \ac{SAHI} tends to generally improve the scores. However here this is especially pronouced, since the augmentations the model was build with were optimized for \ac{SAHI}. Otherwise using a Border Ignore Size improves the score further. However this improvement in score mostly stems from the fact that the testing data almost exclusively contains the marker in the middle of the image. Depending on real world usage this may not always be the case. The squareness thereshold improves the score, but only if it is used alone. 

The foreground mask for these tests was obtained by manually applying the IS-Net from Qin et al. \cite{qin2022} on the testset images. The usage of these masks lead to the highest score on the ensemble testset. However it is debatable if it would be worth the uplift in score for the possibly increased memory usage and inference time.

\begin{table}
  \begin{tabular}{ c c }
   Inference Filter & Pascal VOC 2010 \ac{mAP} in \% \\ 
   \hline
   Min Confidence 50\% & 20.52 \\
   Min Confidence 50\% + \ac{SAHI} & 70.80 \\
   Min Confidence 50\% + \ac{SAHI} + Squareness Thershold 50\% & 70.95 \\
   Min Confidence 50\% + \ac{SAHI} + Border Ignore Size 10\% & 72.24 \\
   Min Confidence 50\% + \ac{SAHI} + Border Ignore Size 10\% + Squareness Thershold 50\% & 72.24 \\
   Min Confidence 50\% + \ac{SAHI} + Border Ignore Size 20\% & 73.72 \\
   Min Confidence 50\% + \ac{SAHI} + Border Ignore Size 30\% & 73.29 \\
   Min Confidence 50\% + \ac{SAHI} + Foreground Mask & 74.12 \\
   \hline
  \end{tabular}
...
This section describes the data used and the results of the evalation of the appraoches for the point detection part of the pipeline.
...
As seen in \autoref{tab:pet_valdata_results}, yeah
