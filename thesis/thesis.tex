...
As part of the development of a new kind of \ac{POCT} for medical purposes, this work addresses the construction of a detection system for partially invisible \ac{ArUco} marker structures on flexible photonic crystals within tape. The usecase for this detection is the reconstruction of the camera pose from the markers on the smartphone of the user. The detection task is split into two parts. 

The first part addresses the detection of areas of interest in the image using object detection networks. Due to a shortage of training data, a number of augmentations are defined and prebuilt into the dataset, such that the same augmentation can be tested with many model architectures without changing the dataloader for each one. The impact of the augmentations on the resulting \ac{mAP} scores are then measured using ensemble runs and hyperparameter optimizations. Furthermore, inference techniques such as \ac{SAHI} are evaluated and used to increase the detection performance. Despite dissimilarity of training and test data for this part, the augmentation, training and inference techniques can increase the VOC 2010 \ac{mAP} score by more than 65\% when used with yolov5s.
...
Random Crop v2 had the worst impact on scores, as seen in \autoref{fig:zero_rc2_plot}. The correlation for the VOC 2010 score here is -43\% and for COCO is -30\%. This may occur since the second version zooms too far into the image and since such large markers do not appear in the test set. Default Random Crop did not show similar impact on the scores.
...
Based on the findings of Treven et al. \cite{terven2023comprehensive}, \ac{YOLO}v7 models may be promising, since they achieved a better score with them than with \ac{YOLO}v5. However, the COCO \ac{mAP} score difference was 1\%.
