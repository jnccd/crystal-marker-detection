...
This work focuses on the detection of the \ac{ArUco} fiducial markers that were etched into tape using neural networks, which are planned to be used on smart phones, such that the tapes can be used as \acp{POCT} by patients. Since smart phones tend to have smaller memory and not much space or energy for large \acp{GPU}, smaller networks are preferably used.
