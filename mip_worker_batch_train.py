...
for i in range(args.num_gpus):
    print(f"Starting run {i} of {args.num_gpus} with " + f"screen -dmS worker{i} \"with_gpu -n 1 sudo mip-docker-run --rm --gpus '\"device=$CUDA_VISIBLE_DEVICES\"' {args.image} {args.command} -wi {i} -wc {args.num_gpus}\"")
