...
for img in pyr_imgs[3:5]:
    ...
    for ppoint in promising_points:
        ...
        # bin_edges {bin_edges}, 
        print(f'img_id {img_i}, ppoint {ppoint}, hist {hist}, hist_diff {hist_diff}, bbox, {(x_min, y_min, x_max, y_max)}')
        
    # --- Final markeryness computation
    split_markerynesses = [x[1] for x in markerynesses]
    max_markeryness = max(split_markerynesses) / 2
    ...
    for m in markerynesses:
        current_markeryness = ((m[1] - min_markeryness) / mm_diff_markeryness) * 255
        current_markeryness = 255 if current_markeryness > 255 else current_markeryness
        current_markeryness = 255 - current_markeryness
        cv2.rectangle(img_draw, (m[0][0],m[0][1],1,1), (0,0, current_markeryness))
    cv2.imwrite(str(root_dir / f'{img_i}_hist_markerynesses.png'), img_draw)
